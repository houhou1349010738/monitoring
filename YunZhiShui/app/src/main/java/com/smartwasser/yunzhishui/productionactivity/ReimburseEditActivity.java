...
/**
 * Created by huangmengjie on 2016/8/30 0030.
 */
public class ReimburseEditActivity extends BaseActivity implements View.OnClickListener,HttpLoader.ResponseListener,AbsListView.OnScrollListener{
    ...
    private  String ywlx,sqsj,ywCode;
    ...
    protected void initListener() {
        ...
        ed_addreimbures_sqbxje.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (s.toString() != null && !"".equals(s.toString()) && !"null".equals(s.toString())) {
                    ed_addreimbures_sqjedx.setText(MoneyChange.digitUppercase(Double.parseDouble(s.toString())));
                }
            }
        });
    }
        ...
        switch (v.getId()) {
            ...
            case R.id. but_addreimbures_save:
                ...
                if("".equals(xmmc)||"null".equals(xmmc)||xmmc==null){
                    Toast.makeText(ReimburseEditActivity.this, "请选择项目名项目", Toast.LENGTH_SHORT).show();
                    return;
                }
                if("".equals(sqsj)||sqsj==null||"null".equals(sqsj)){
                    Toast.makeText(ReimburseEditActivity.this, "请选择申请时间", Toast.LENGTH_SHORT).show();
                    return;
                }
                if("".equals(ywlx)||ywlx==null||"null".equals(ywlx)){
                    Toast.makeText(ReimburseEditActivity.this, "请选择业务类型", Toast.LENGTH_SHORT).show();
                    return;
                }
                if("".equals(fplx)||fplx==null||"null".equals(fplx)){
                    Toast.makeText(ReimburseEditActivity.this, "请选择发票类型", Toast.LENGTH_SHORT).show();
                    return;
                }

                if(fplx.equals("增值税专用发票")){
                    prams.put("claimSum1",je);
                    fplx="ZY";
                    if("".equals(je) || je == null || "null".equals(je)){
                        Toast.makeText(ReimburseEditActivity.this, "请填写金额", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    if("".equals(sl) || sl == null || "null".equals(sl)){
                        Toast.makeText(ReimburseEditActivity.this, "请填写税率", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    if("".equals(se) || se == null || "null".equals(se)){
                        Toast.makeText(ReimburseEditActivity.this, "请填写税额", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    ...
                    if(m!=Double.parseDouble(je)){
                        Toast.makeText(ReimburseEditActivity.this, "业务科目金额和申请金额不相等", Toast.LENGTH_SHORT).show();
                        return;
                    }
                ...
                if("".equals(skdw) || skdw == null || "null".equals(skdw)){
                    Toast.makeText(ReimburseEditActivity.this, "请填写收款单位", Toast.LENGTH_SHORT).show();
                    return;
                }
                if("".equals(sqbxje) ||sqbxje == null || "null".equals(sqbxje)){
                    Toast.makeText(ReimburseEditActivity.this, "请填写申请金额", Toast.LENGTH_SHORT).show();
                    return;
                }
                if("".equals(bxsm) ||bxsm == null || "null".equals(bxsm)){
                    Toast.makeText(ReimburseEditActivity.this, "请填写报销说明", Toast.LENGTH_SHORT).show();
                    return;
                }
                if("".equals(dyje) ||dyje == null || "null".equals(dyje)){
                    Toast.makeText(ReimburseEditActivity.this, "请填写对应金额", Toast.LENGTH_SHORT).show();
                    return;
                ...
                prams.put("claimYear1",meDetails.getData().getPo().getClaimYear()+"");
                prams.put("invoiceType",fplx);
                prams.put("xfdw",skdw);
                prams.put("claimSumValue",sqbxje);
                prams.put("claimSumcn",sqdxje);
                prams.put("claimRealuser",meDetails.getData().getPo().getClaimRealuser());
                ...
                prams.put("claimId",event.getClaimId());
                HttpLoader.get(ConstantsYunZhiShui.URL_TYPEMASTERUPDATE, prams,
                        SaveResponse.class, ConstantsYunZhiShui.REQUEST_CODE_TYPEMASTERUPDATE, this, false).setTag(this);
                break;
        }
    }
    @Override
    public void onGetResponseSuccess(int requestCode, RBResponse response) {
        if (requestCode == ConstantsYunZhiShui.REQUEST_CODE_TYPEMASTERPAYVIEW
            ...
            if("00000".equals(meDetails.getErrorCode())){
                tv_addreimbures_sqr.setText("申请人:"+meDetails.getData().getPo().getClaimRealuser());
                tv_addreimbures_dh.setText("NO:"+meDetails.getData().getPo().getClaimYear()+"");
                ed_addreimbures_xmmc.setText(meDetails.getData().getUnitName());
                ed_addreimbures_ywlx.setText(meDetails.getData().getPo().getClaimtypeId());
                ed_addreimbures_skdw.setText(meDetails.getData().getPo().getXfdw());
                String fple="";
                if(meDetails.getData().getInv()!=null){
                    fple=meDetails.getData().getInv().getInvoiceType();
                    if("PT".equals(fple)){
                        ed_addreimbures_fplx.setText("增值税普通发票");
                    }else if("ZY".equals(fple)){
                ...
                if("ZY".equals(fple)){
                    relative.setVisibility(View.VISIBLE);
                    ed_addreimbures_je.setText(meDetails.getData().getInv().getInvoiceMoney()+"");
                    ed_addreimbures_sl.setText(meDetails.getData().getInv().getInvoiceTaxes()+"");
                    ed_addreimbures_se.setText(meDetails.getData().getInv().getInvoiceTax() + "");
                }else{
                    relative.setVisibility(View.GONE);
                }
                ed_addreimbures_sqsj.setText(meDetails.getData().getPo().getClaimTime());
                ed_addreimbures_sqbxje.setText(meDetails.getData().getPo().getClaimSum()+"");
                ed_addreimbures_sqjedx.setText(MoneyChange.digitUppercase(meDetails.getData().getPo().getClaimSum()));
                ed_addreimbures_bxsm.setText(meDetails.getData().getPo().getClaimDesc());
                double  je=0;
                for(int y=0;y<meDetails.getData().getFeeSubjectRelation().size();y++){
                            mData.add(new ItemBean());
                    final ItemBean itemObj=mData.get(y);
                    listCode.add(meDetails.getData().getFeeSubjectRelation().get(y).getProfessionSubject());
                    itemObj.setTextKemu(meDetails.getData().getFeeSubjectRelation().get(y).getIndeName());
                    itemObj.setTextSyz(meDetails.getData().getFeeSubjectRelation().get(y).getMinValue());
                    itemObj.setTextJhz(meDetails.getData().getFeeSubjectRelation().get(y).getMaxValue());
                    itemObj.setTextJe(meDetails.getData().getFeeSubjectRelation().get(y).getFeeAmount()+"");
                          /**把得到的金额相加*/
                    je+=meDetails.getData().getFeeSubjectRelation().get(y).getFeeAmount();
                }
        ...
        if (requestCode == ConstantsYunZhiShui.REQUEST_CODE_TYPEMASTERPAYPAYBOXLIST
            ...
            if("00000".equals(mProject.getErrorCode())){
                if(c==1) {
                    for (int i = 0; i < mProject.getData().getListTree().size(); i++) {
                        if (meDetails.getData().getUnitName().equals(mProject.getData().getListTree().get(i).getText())) {
        ...
        if (requestCode == ConstantsYunZhiShui.REQUEST_CODE_TYPEMASTERYWKMJY
            ...
            if ("00000".equals(IsFill.getErrorCode())) {
                if (IsFill.getData().getSize() >= 1) {
                    ...
                    Toast.makeText(ReimburseEditActivity.this, "本月无该项目预算计划", Toast.LENGTH_SHORT).show();
                }
            }
        ...
        if (requestCode == ConstantsYunZhiShui.REQUEST_CODE_TYPEMASTERUPDATE
                && response instanceof SaveResponse) {
            SaveResponse msave = (SaveResponse) response;
            Log.i("00000",msave.getErrorCode());
            if("00000".equals(msave.getErrorCode())){
                /**跳转到首页*/
                Intent intent=new Intent(ReimburseEditActivity.this,ReimbursementActivity.class);
    ...
    class MyAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, final ViewGroup parent) {
            if(convertView==null){
                convertView=View.inflate(ReimburseEditActivity.this,R.layout.item_yewukemu,null);
            ...
            holder.but_xuanze.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if ("".equals(xmmc) || "null".equals(xmmc) || xmmc == null) {
                        Toast.makeText(ReimburseEditActivity.this, "请选择项目名称", Toast.LENGTH_SHORT).show();
                        return;
                        ...
                        HttpLoader.get(ConstantsYunZhiShui.URL_TYPEMASTERYWKMJY, prams,
                                IsFillResponse.class, ConstantsYunZhiShui.REQUEST_CODE_TYPEMASTERYWKMJY, ReimburseEditActivity.this, false).setTag(this);
                    }
                }
            });
            return convertView;
    ...
    class MyAdapters extends BaseAdapter{
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ViewHolder holder;
            if(convertView==null){
                convertView=View.inflate(ReimburseEditActivity.this,R.layout.item_business,null);
                holder=new ViewHolder();
    ...
    class MyBusinessAdapter extends BaseAdapter{
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                holder = new ViewHolder();
                convertView = View.inflate(ReimburseEditActivity.this, R.layout.item_search, null);
